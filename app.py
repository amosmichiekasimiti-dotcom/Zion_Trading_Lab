...
class ZionOmniEngine:
    ...
    def get_universal_signal():
        # Universal Asset Categories: maneuvering through all indices
        categories = [
        ...
        # 10% Individual Digit Competition Rule
        freqs = {d: random.uniform(6.5, 15.5) for d in range(10)}
        ...
        # Applying the 10% Individual Floor per Digit
        if st == "UNDER":
            ...
            # Logic: If at least 3 digits in the group are > 10%
            action = st; status = "STRIKE"; is_ready = True

        life = random.randint(45, 65)
        acc = f"{random.uniform(98.8, 99.9):.1f}%" if is_ready else "N/A"
        
        voice = f"Zion Apex. Market: {selected_cat}. "
        voice += f"Command: {action}. Execute." if is_ready else "Maneuvering. Cluster is currently weak."
...
<head>
    ...
    <style>
        ...
        .header-stack { position: sticky; top: 0; z-index: 1000; width: 100%; background: var(--bg); }
        ...
        .market-tag { color: var(--cyan); font-size: 11px; text-transform: uppercase; letter-spacing: 2px; }
        ...
        .status-msg { font-size: 35px; font-weight: 900; margin: 20px 0; }
        .ready { color: var(--green); text-shadow: 0 0 10px var(--green); }
        .btn-exec { background: var(--green); color: white; padding: 18px; display: block; text-decoration: none; border-radius: 12px; font-weight: 900; font-size: 18px; }
        .wa-float { position: fixed; bottom: 25px; right: 20px; background: #25d366; width: 55px; height: 55px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 30px; z-index: 1000; }
    </style>
</head>
<body>
    <div class="header-stack">
        <div class="navbar">
            ...
            <div style="font-weight:900; font-size:18px;">ZION <span style="color:var(--cyan)">AI</span></div>
            <div style="background:var(--red); padding:4px 10px; border-radius:6px; font-size:10px; font-weight:bold;">LIVE</div>
        </div>
        <div class="scroll-nav">
            <a href="/?cat=DASHBOARD" class="nav-link">Dashboard</a>
            <a href="/?cat=SIGNAL" class="nav-link active">Live Signal</a>
    ...
    <div class="container">
        {% if mode == 'SIGNAL' %}
            ...
            <div style="margin-top:20px; font-size:12px; color:var(--cyan);">REFRESHING IN: <span id="clock">{{ s.life }}</span>s</div>
        {% else %}
            <div style="margin-top:50px;">
                <i class="fa-solid fa-microchip" style="font-size: 60px; color: var(--cyan); margin-bottom: 20px;"></i>
                <h2 style="letter-spacing:2px;">ZION APEX SYSTEM</h2>
                <a href="/?cat=SIGNAL" style="color:white; border: 1px solid var(--cyan); padding: 15px 40px; display: inline-block; text-decoration:none; border-radius: 10px; font-weight:bold;">LAUNCH SCANNER</a>
            </div>
        {% endif %}
    </div>
